      ...
      <div v-else class="flex space-x-4 h-full">
        ...
        >
          ...
          <draggable
            ...
            group="tasks"
            item-key="id"
            class="p-4 space-y-4 h-full min-h-[100px]"
            @end="handleDragEnd"
          >
...
// --- 4. FUNCIÓN handleDragEnd CORREGIDA Y ROBUSTA ---
const handleDragEnd = (event: SortableEvent) => {
  ...
  // Si no hay datos clave o la tarea no cambió de columna, no hacemos nada
  if (!taskId || !newStatus || !originalStatus || newStatus === originalStatus) {
    return;
  }

  // Buscamos la tarea en el array original (no en el grupo) para asegurar que la encontramos
