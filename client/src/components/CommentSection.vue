  ...
  <div class="mt-6 pt-6 border-t dark:border-gray-700">
    ...
    <div class="space-y-4 max-h-60 overflow-y-auto custom-scrollbar pr-2">
      ...
      <div v-else-if="taskStore.currentTaskComments.length === 0">
        <p class="text-gray-500 text-sm">No hay comentarios todavía. ¡Sé el primero en añadir uno!</p>
      </div>
      <div v-else v-for="comment in taskStore.currentTaskComments" :key="comment.id" class="flex items-start space-x-3">
        ...
        <div class="flex-1 bg-gray-100 dark:bg-gray-700 rounded-lg px-4 py-2">
          <div class="flex items-center justify-between">
            <p class="font-semibold text-sm text-dark-purple dark:text-light">{{ comment.user.full_name }}</p>
            <p class="text-xs text-gray-500">{{ formatDistanceToNow(new Date(comment.created_at), { addSuffix: true, locale: es }) }}</p>
          </div>
          <p class="text-sm text-gray-800 dark:text-gray-300 mt-1 whitespace-pre-wrap">{{ comment.content }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="!isViewOnly" class="mt-4">
      <form @submit.prevent="handlePostComment">
        <textarea
          v-model="newComment"
          rows="3"
          class="w-full p-2 border rounded-md dark:bg-gray-900 dark:border-gray-600 focus:ring-accent focus:border-accent"
